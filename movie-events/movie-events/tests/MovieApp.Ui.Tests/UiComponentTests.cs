...
public sealed class SectionHeaderInstanceTests
{
    [Fact]
    public void Properties_CanBeSetAndRetrieved()
    {
        ...
        header.ActionText = "View More";

        Assert.Equal("Upcoming Events", header.Title);
        Assert.Equal("View More", header.ActionText);
    }
}

public sealed class EventSearchBoxInstanceTests
{
    [Fact]
    public void Properties_CanBeSetAndRetrieved()
    {
        var box = new EventSearchBox();

        box.SearchText = "Action Movies";
        box.PlaceholderText = "Type here...";

        Assert.Equal("Action Movies", box.SearchText);
        Assert.Equal("Type here...", box.PlaceholderText);
    }
}

public sealed class EventSortSelectorInstanceTests
{
    [Fact]
    public void Properties_CanBeSetAndRetrieved()
    {
        var selector = new EventSortSelector();

        selector.SelectedSortOption = MovieApp.Core.EventLists.EventSortOption.PriceDescending;
        selector.PlaceholderText = "Select sorting...";

        Assert.Equal(MovieApp.Core.EventLists.EventSortOption.PriceDescending, selector.SelectedSortOption);
        Assert.Equal("Select sorting...", selector.PlaceholderText);
    }
}

public sealed class EventCardInstanceTests
{
    [Fact]
    public void DiscountPercentage_UpdatesComputedProperties()
    {
        var card = new EventCard();

        card.DiscountPercentage = 15;

        Assert.True(card.HasDiscount);
        Assert.Equal(Visibility.Visible, card.DiscountBadgeVisibility);
        Assert.Equal("-15%", card.DiscountBadgeText);

        card.DiscountPercentage = 0;

        Assert.False(card.HasDiscount);
        Assert.Equal(Visibility.Collapsed, card.DiscountBadgeVisibility);
    }

    [Fact]
    public void IsJoined_UpdatesVisibilityProperties()
    {
        var card = new EventCard();

        card.IsJoined = true;
        Assert.Equal(Visibility.Collapsed, card.JoinButtonVisibility);
        Assert.Equal(Visibility.Visible, card.JoinedStatusVisibility);

        card.IsJoined = false;
        Assert.Equal(Visibility.Visible, card.JoinButtonVisibility);
        Assert.Equal(Visibility.Collapsed, card.JoinedStatusVisibility);
    }

    [Fact]
    public void IsFavorited_UpdatesFavoriteIconGlyph()
    {
        var card = new EventCard();

        card.IsFavorited = true;
        Assert.Equal("\uEB52", card.FavoriteIconGlyph);
    ...
    [Fact]
    public void ModelProperty_PopulatesTextPropertiesCorrectly()
    {
        var card = new EventCard();
        var testEvent = new Event
        {
            ...
            LocationReference = "Main Theater",
        ...
        card.Model = testEvent;

        Assert.Equal("Integration Test Event", card.TitleText);
        Assert.Equal("Integration Desc", card.DescriptionText);
        Assert.Equal("Main Theater", card.LocationText);
        Assert.Equal("50/100", card.CapacityText);
    }
}